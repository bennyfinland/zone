	...
	$("#commentForm").validate({
		...
		rules: {
			 	title:{
					...
					minlength: 2,
					remote: {
						url: "<?php echo base_url();?>index.php/zone/check_article_title_edit/<?php echo $this->uri->segment(3);?>",
						type: "post",
						data: {
							title: function(){
								return $("#title").val();
...
<?php foreach($article_edit->result() as $row):?>	
	<div class="c_form">
		<?php 
			...
			echo form_open("zone/update_edit",$attributes);
		?>
			<table width="100%" cellspacing="4" cellpadding="4" class="infotable">
				<tbody>
					<tr>
						<td>
							你的分类	<?php echo $row->class_name;?><br><br>
								
							文章标题<input type="text" size="60" value="<?php echo $row->article_title;?>" name="title" id="title" class="t_input">
									<input type="hidden" value="<?php echo $row->article_title;?>" name="old_title" id="old_title" class="t_input">
									<input type="hidden" value="<?php echo $row->class_name;?>" name="class_name" id="class_name" class="t_input">
