		...
		<tbody><tr>
			<?php $i=0; foreach($gruop->result() as $row):?>
				...
				<td style="padding-bottom: 20px;" >
					<div class="album_bg">
						<?php echo '<a href="'.base_url().'index.php/zone/show_photo/'.$this->uri->segment(3).'/'.$row->gruop_name.'/'.$row->gruop_username.'">' ?>  
					...
					<p>
						...
						<span class="gray">
							&nbsp;(<?php
										$photo_total=$this->db->query("SELECT * FROM photo WHERE photo_username='$row->gruop_username' and gruop_name='$row->gruop_name'");
								   		echo $photo_total->num_rows();
									?>)
						</span>	
					</p>
					<p class="gray">
						<?php echo $row->gruop_username;?>
					</p>
				</td>
			<?php $i++; endforeach; ?>
		</tr></tbody>
